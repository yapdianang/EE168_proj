...
k = 1;
for height = 900:-5:100
    relief = (conv2(data, [1 -0.5; -0.5 0], 'same') + 1) ./ 2;
    relief(relief < -50) = -50;
    relief(relief > 50) = 50;
    relief = relief - min(min(relief));
    relief = relief / max(max(relief)) * 255;
    relief = relief .* (255 - data + 128)/255;

    perspective = perspective_projection(data, relief, 2000, height);
    imshow(imrotate(perspective, 180)')
    M(k) = getframe(gcf);
    k = k + 1;
end
